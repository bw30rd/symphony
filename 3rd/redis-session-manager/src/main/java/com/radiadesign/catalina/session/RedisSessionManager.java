...
public class RedisSessionManager extends ManagerBase implements Lifecycle {
  ...
  private int port_val = 6379;
  protected String database = "0";
  ...
  private static String getParam(String val) {
    if (val == null) {
      return val;
    }
    val = val.trim();
    ...
    if (val.startsWith("${") && val.endsWith("}")) {
      refkey = val.substring(2, val.length() - 1);
    } else if (val.startsWith("$")){
      refkey = val.substring(1);
    }
    if (refkey != null) {
      refval = System.getenv(refkey);
      if (refval == null) {
        refval = System.getProperty(refkey);
      }
      if (refval != null) {
        refval = getParam(refval);
      }
    }
    return refval != null ? refval : val;
  }
  private static int getIntParam(String val) {
    val = getParam(val);
  ...
  private void prepareParams() {
    host = getParam(host);
    port_val = getIntParam(port);
    database_val = getIntParam(database);
  ...
  private void initializeDatabaseConnection() throws LifecycleException {
    try {
      ...
      connectionPool = new JedisPool(new JedisPoolConfig(), host, port_val, timeout, password);
    } catch (Exception e) {
      e.printStackTrace();
