...
public final class CacheFactory {
    ...
    public static synchronized void removeAll() {
        RuntimeEnv runtime = getRuntimeEnv();
            ...
            if (null == ret) {
                switch (getRuntimeEnv()) {
                    case LOCAL:
                        ...
                        break;
                    case REDIS:
                    	 final Class<Cache<String, ?>> bwaeRedisCache = (Class<Cache<String, ?>>) Class.forName(
                                 "org.b3log.latke.cache.bwae.redis.RedisCache");

                         ret = bwaeRedisCache.getConstructor(String.class).newInstance(cacheName);
                         break;
                    default:
                        throw new RuntimeException("Latke runs in the hell.... Please set the enviornment correctly");
                }
    ...
    private static RuntimeEnv getRuntimeEnv() {
    	RuntimeEnv env = Latkes.getRuntime("cache");
    	if (RuntimeEnv.REDIS.equals(env)) {
    ...
    public static boolean IsRedisConfigured() {
    	...
    	for (String k : System.getenv().keySet()) {
    		if (k.startsWith("AE_BS_REDIS_")) {
				if (k.endsWith("_HOST")) {
					host = System.getenv(k);
				} else if (k.endsWith("_PORT")) {
					port = Integer.parseInt(System.getenv(k));
				}
			}
    	}
