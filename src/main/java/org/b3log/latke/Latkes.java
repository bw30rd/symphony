...
public final class Latkes {
    ...
    public static String getServerHost() {
        if (null == serverHost) {
            ...
            if (aeEnvType != null && !aeEnvType.isEmpty()) {
                String host = ".bwae.org";
                if ("DEVELOP".equals(aeEnvType)) {
                    host = ".aedev" + host;
                } else if ("TEST".equals(aeEnvType)) {
                    host = ".aetest" + host;
                }
                String namespace = System.getenv("AE_NAMESPACE");
                serverHost = namespace + host;
            } else {
    ...
    public static String getServer() {
        if (null == server) {
            ...
            final StringBuilder serverBuilder = new StringBuilder(scheme).append("://").append(getServerHost());
            final String port = getServerPort();
            if (!Strings.isEmptyOrNull(port)) {
                boolean isDefaultPort = ("https".equals(scheme) && port.equals("443"))
                        || ("http".equals(scheme) && port.equals("80"));
                if (!isDefaultPort) {
...
//            staticServePath = getStaticServer() + getStaticPath();
    ...
    public static String getLocalProperty(final String key) {
        String value = LOCAL_PROPS.getProperty(key);
        if (value != null) {
            Matcher m = envPattrn.matcher(value);
            if (m != null) {
                while (m.find()) {
                    String t = m.group(1);
                    if (t != null) {
                        String sysKey = t.replaceAll("^\\$\\{(.+)\\}$", "$1");
                        String sysValue = System.getProperty(sysKey);
                        if (sysValue == null) {
                            sysValue = System.getenv(sysKey);
                        }
                        if (sysValue == null) {
                            sysValue = LOCAL_PROPS.getProperty(sysKey);
                        }
                        if (sysValue != null) {
                            value = value.replace(t, sysValue);
                        }
                        
                    }
                }
                LOCAL_PROPS.setProperty(key, value);
            }
        }
        return value;
    }
    private static Pattern envPattrn = Pattern.compile("(\\$\\{[^\\}]+\\})");
