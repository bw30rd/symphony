...
public class UserMgmtService {
	 ...
	 * @param ip
	 *            the specified IP, could be "" if the {@code onlineFlag} is
	 *            {@code false}
	 * @param onlineFlag
	 *            the specified online flag
	 ...
	 *            the specified request json object (user), for example, "oId": "",
	 *            "userNickname": "", "userTags": "", "userURL": "", "userQQ": "",
	 *            "userIntro": "", "userAvatarType": int, "userAvatarURL": "",
	 ...
	 * @param requestJSONObject
	 *            the specified request json object (user), for example, "oId": "",
	 *            "userB3Key": "", "userB3ClientAddArticleURL": "",
	 *            "userB3ClientUpdateArticleURL": "", "userB3ClientAddCommentURL":
	 *            "", "syncWithSymphonyClient": boolean // optional, default to
	 *            false
	 * @throws ServiceException
	 *             service exception
	 */
	public void updateSyncB3(final JSONObject requestJSONObject) throws ServiceException {
		...
		try {
			...
			oldUser.put(UserExt.USER_B3_CLIENT_ADD_ARTICLE_URL,
					requestJSONObject.optString(UserExt.USER_B3_CLIENT_ADD_ARTICLE_URL));
			oldUser.put(UserExt.USER_B3_CLIENT_UPDATE_ARTICLE_URL,
					requestJSONObject.optString(UserExt.USER_B3_CLIENT_UPDATE_ARTICLE_URL));
			oldUser.put(UserExt.USER_B3_CLIENT_ADD_COMMENT_URL,
					requestJSONObject.optString(UserExt.USER_B3_CLIENT_ADD_COMMENT_URL));
	 ...
	 *
	 * @param requestJSONObject
	 *            the specified request json object, for example, "userName": "",
	 *            "userEmail": "", "userPassword": "", // Hashed "userLanguage": "",
	 *            "userAppRole": int, // optional, default to 0 "userRole": "", //
	 *            optional, uses {@value Role#ROLE_ID_C_DEFAULT} instead if not
	 *            specified "userStatus": int, // optional, uses
	 *            {@value UserExt#USER_STATUS_C_NOT_VERIFIED} instead if not
	 *            specified "userGuideStep": int // optional, uses
	 *            {@value UserExt#USER_GUIDE_STEP_UPLOAD_AVATAR} instead if not
	 *            specified ,see {@link User} for more details
	 * @return generated user id
	 * @throws ServiceException
	 *             if user name or email duplicated, or repository exception
	 */
	public synchronized String addUser(final JSONObject requestJSONObject) throws ServiceException {
		...
		try {
			...
			final String userNickname = requestJSONObject.optString(User.USER_NICKNAME);
			final String userCountry = requestJSONObject.optString("userCountry");
			...
			user.put(UserExt.USER_COUNTRY, userCountry);
			...
			user.put(UserExt.USER_GUIDE_STEP,
					requestJSONObject.optInt(UserExt.USER_GUIDE_STEP, UserExt.USER_GUIDE_STEP_UPLOAD_AVATAR));
			...
			if (toUpdate) {
				...
				try {
					...
					if (Symphonys.getBoolean("qiniu.enabled")) {
						...
						uploadManager.put(bytes, "avatar/" + ret, auth.uploadToken(Symphonys.get("qiniu.bucket")), null,
								"image/jpeg", false);
						user.put(UserExt.USER_AVATAR_URL,
								Symphonys.get("qiniu.domain") + "/avatar/" + ret + "?" + new Date().getTime());
					} else {
				...
				final int memberCount = memberCntOption.optInt(Option.OPTION_VALUE) + 1; // Updates stat. (member count
																							// +1)
			...
			if (UserExt.USER_STATUS_C_VALID == status) {
				 ...
				 */
				pointacquireMgmtService.acquire(ret, Pointtransfer.ID_C_SYS, Pointtransfer.TRANSFER_TYPE_C_INIT,
						Pointtransfer.TRANSFER_SUM_C_INIT, ret, System.currentTimeMillis());
	 ...
	 *            the given user id
	 * @param user
	 *            the specified user, contains the new email
	 * @throws ServiceException
	 *             service exception
	 */
	 ...
	 *            the given user id
	 * @param user
	 *            the specified user, contains the new username
	 * @throws ServiceException
	 *             service exception
	 */
