...
public class DataModelService {
	 ...
	 *             exception
	 */
	public void fillRelevantArticles(final int avatarViewMode, final Map<String, Object> dataModel,
		...
		try {
			dataModel.put(Common.SIDE_RELEVANT_ARTICLES, articleQueryService.getRelevantArticles(avatarViewMode,
					article, Symphonys.getInt("sideRelevantArticlesCnt")));
		} finally {
			Stopwatchs.end();
		}
	}
	...
	public void fillLatestCmts(final Map<String, Object> dataModel) throws Exception {
		...
		try {
			// dataModel.put(Common.SIDE_LATEST_CMTS,
			// commentQueryService.getLatestComments(Symphonys.getInt("sizeLatestCmtsCnt")));
	...
	public void fillRandomArticles(final int avatarViewMode, final Map<String, Object> dataModel) throws Exception {
		...
		try {
			...
			if (fetchSize > 0) {
				dataModel.put(Common.SIDE_RANDOM_ARTICLES,
						articleQueryService.getRandomArticles(avatarViewMode, fetchSize));
			} else {
	...
	private void fillHeader(final HttpServletRequest request, final HttpServletResponse response,
		...
		dataModel.put("serveSecheme", Latkes.getServerScheme());
	...
	public void fillHeaderAndFooter(final HttpServletRequest request, final HttpServletResponse response,
		...
		String wsScheme = Symphonys.get("websocket.scheme");
		String aeEnvType = System.getenv("AE_ENV_TYPE");
		if (aeEnvType != null && "PRODUCT".equals(aeEnvType)) {
			wsScheme = "wss";
		}

		dataModel.put(Common.WEBSOCKET_SCHEME, wsScheme);
	}
	...
	private void fillPersonalNav(final HttpServletRequest request, final HttpServletResponse response,
		...
		try {
			...
			dataModel.put(Common.IS_DOMAIN_ADMIN, false);
			...
			final List<JSONObject> roles = roleQueryService.getRoles(userRole);
			final String roleNames = roleQueryService.getRoleNames(userRole);
			...
			for (JSONObject role : roles) {
				if (Role.ROLE_ID_C_ADMIN.equals(userRole)) {
					dataModel.put(Common.IS_ADMIN_LOGGED_IN, true);
				}
				if (role.optString(Role.ROLE_NAME).contains(Role.ROLE_ID_C_DOMAIN_ADMIN)) {
					dataModel.put(Common.IS_DOMAIN_ADMIN,true);
				}
			}
			...
			final long followingArticleCnt = followQueryService.getFollowingCount(userId,
			...
			int watchingArticleCnt = followMgmtService.getFollowCnt(curUser, Follow.FOLLOWING_TYPE_C_ARTICLE_WATCH);

			curUser.put(Common.WATCHING_ARTICLE_CNT, watchingArticleCnt);
			...
			curUser.put(Role.ROLE_NAME, roleNames);

			// final int unreadNotificationCount =
			// notificationQueryService.getUnreadNotificationCount(curUser.optString(Keys.OBJECT_ID));
			...
			dataModel.put(Liveness.LIVENESS,
					(float) (Math.round((float) currentLiveness / livenessMax * 100 * 100)) / 100);
		} finally {
			Stopwatchs.end();
		}
	}
	...
	private void fillTrendTags(final Map<String, Object> dataModel) throws Exception {
		...
		try {
			// dataModel.put(Common.NAV_TREND_TAGS,
			// tagQueryService.getTrendTags(Symphonys.getInt("trendTagsCnt")));
