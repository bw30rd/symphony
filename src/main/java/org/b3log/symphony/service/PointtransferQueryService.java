...
public class PointtransferQueryService {
	...
	/**
	 * Gets the latest pointtransfers with the specified user id, type and fetch
	 * size.
	 *
	 * @param userId
	...
	public List<JSONObject> getTopBalanceUsers(final int avatarViewMode, final int fetchSize) {
		...
		final Query query = new Query().addSort(UserExt.USER_POINT, SortDirection.DESCENDING).setCurrentPageNum(1)
				.setPageSize(fetchSize).setFilter(new PropertyFilter(UserExt.USER_JOIN_POINT_RANK, FilterOperator.EQUAL,
	...
	public List<JSONObject> getTopConsumptionUsers(final int avatarViewMode, final int fetchSize) {
		...
		final Query query = new Query().addSort(UserExt.USER_USED_POINT, SortDirection.DESCENDING).setCurrentPageNum(1)
				.setPageSize(fetchSize).setFilter(new PropertyFilter(UserExt.USER_JOIN_USED_POINT_RANK,
		...
		try {
			...
			for (int i = 0; i < records.length(); i++) {
				...
				if (("3".equals(typeStr) && userId.equals(toId)) || ("5".equals(typeStr) && userId.equals(fromId))
						|| ("9".equals(typeStr) && userId.equals(toId)) || ("14".equals(typeStr) && userId.equals(toId))
				...
				case Pointtransfer.TRANSFER_TYPE_C_ADD_ARTICLE:
					...
					final String addArticleLink = "<a href=\"" + addArticle.optString(Article.ARTICLE_PERMALINK) + "\">"
				...
				case Pointtransfer.TRANSFER_TYPE_C_UPDATE_ARTICLE:
					...
					final String updateArticleLink = "<a href=\"" + updateArticle.optString(Article.ARTICLE_PERMALINK)
				...
				case Pointtransfer.TRANSFER_TYPE_C_ADD_COMMENT:
					...
					final String commentArticleLink = "<a href=\"" + commentArticle.optString(Article.ARTICLE_PERMALINK)
					...
					if ("3In".equals(typeStr)) {
						...
						final String commenterLink = "<a href=\"/member/" + commenter.optString(User.USER_NAME) + "\">"
								+ commenter.optString(User.USER_NICKNAME) + "</a>";
				...
				case Pointtransfer.TRANSFER_TYPE_C_ARTICLE_REWARD:
					...
					final String senderLink = "<a href=\"/member/" + sender.optString(User.USER_NAME) + "\">"
							+ sender.optString(User.USER_NICKNAME) + "</a>";
					...
					final String articleRewardLink = "<a href=\"" + articleReward.optString(Article.ARTICLE_PERMALINK)
				...
				case Pointtransfer.TRANSFER_TYPE_C_COMMENT_REWARD:
					if (userId.equals(toId)) {
						...
						final String userLink14 = "<a href=\"/member/" + user14.optString(User.USER_NAME) + "\">"
								+ user14.optString(User.USER_NICKNAME) + "</a>";
						desTemplate = desTemplate.replace("{user}", userLink14);

						final JSONObject article14 = articleRepository.get(articleId14);
						final String articleLink = "<a href=\"" + article14.optString(Article.ARTICLE_PERMALINK) + "\">"
				...
				case Pointtransfer.TRANSFER_TYPE_C_ARTICLE_THANK:
					...
					// }
					//
					// final String userLink22 = "<a href=\"/member/" +
					// user22.optString(User.USER_NAME) + "\">"
				...
				case Pointtransfer.TRANSFER_TYPE_C_INVITE_REGISTER:
					...
					final String newUserLink = "<a href=\"/member/" + newUser.optString(User.USER_NAME) + "\">"
							+ newUser.optString(User.USER_NICKNAME) + "</a>";
				...
				case Pointtransfer.TRANSFER_TYPE_C_INVITED_REGISTER:
					...
					final String referralUserLink = "<a href=\"/member/" + referralUser.optString(User.USER_NAME)
							+ "\">" + referralUser.optString(User.USER_NICKNAME) + "</a>";
				...
				case Pointtransfer.TRANSFER_TYPE_C_INVITECODE_USED:
					...
					final String newUserLink1 = "<a href=\"/member/" + newUser1.optString(User.USER_NAME) + "\">"
							+ newUser1.optString(User.USER_NICKNAME) + "</a>";
					desTemplate = desTemplate.replace("{user}", newUserLink1);

					break;
				case Pointtransfer.TRANSFER_TYPE_C_VOTE_UP:
					if (userId.equals(fromId)) {
						final JSONObject voteupArticle = articleRepository.get(dataId);

						final String ArticleLink = "<a href=\"" + voteupArticle.optString(Article.ARTICLE_PERMALINK)
								+ "\">" + voteupArticle.optString(Article.ARTICLE_TITLE) + "</a>";
						desTemplate = desTemplate.replace("{article}", ArticleLink);

						final JSONObject articleAuthor = userRepository.get(toId);
						final String articleAuthorLink = "<a href=\"/member/" + articleAuthor.optString(User.USER_NAME)
								+ "\">" + articleAuthor.optString(User.USER_NICKNAME) + "</a>";

						record.put(Common.BALANCE, record.optInt(Pointtransfer.FROM_BALANCE));

						desTemplate = desTemplate.replace("{user}", articleAuthorLink);
					} else {
						desTemplate = langPropsService.get("removedLabel");
					}
					break;
				case Pointtransfer.TRANSFER_TYPE_C_TO_VOTE_UP:
					if (userId.equals(fromId)) {
						final JSONObject voteupArticle2 = articleRepository.get(dataId);

						final String ArticleLink2 = "<a href=\"" + voteupArticle2.optString(Article.ARTICLE_PERMALINK)
								+ "\">" + voteupArticle2.optString(Article.ARTICLE_TITLE) + "</a>";
						desTemplate = desTemplate.replace("{article}", ArticleLink2);

						final JSONObject voteUper = userRepository.get(toId);
						final String voteUperLink = "<a href=\"/member/" + voteUper.optString(User.USER_NAME) + "\">"
								+ voteUper.optString(User.USER_NICKNAME) + "</a>";

						record.put(Common.BALANCE, record.optInt(Pointtransfer.FROM_BALANCE));

						desTemplate = desTemplate.replace("{user}", voteUperLink);
					} else {
						desTemplate = langPropsService.get("removedLabel");
					}
					break;
				...
				case Pointtransfer.TRANSFER_TYPE_C_STICK_ARTICLE:
					...
					final String stickArticleLink = "<a href=\"" + stickArticle.optString(Article.ARTICLE_PERMALINK)
				...
				case Pointtransfer.TRANSFER_TYPE_C_ACCOUNT2ACCOUNT:
					...
					final String userLink = "<a href=\"/member/" + user9.optString(User.USER_NAME) + "\">"
							+ user9.optString(User.USER_NICKNAME) + "</a>";
				...
				case Pointtransfer.TRANSFER_TYPE_C_PERFECT_ARTICLE:
					...
					final String perfectArticleLink = "<a href=\"" + perfectArticle.optString(Article.ARTICLE_PERMALINK)
				...
				if (type == Pointtransfer.TRANSFER_TYPE_C_COMMENT_REWARD
						|| (type == Pointtransfer.TRANSFER_TYPE_C_VOTE_UP && !userId.equals(fromId))
						|| (type == Pointtransfer.TRANSFER_TYPE_C_TO_VOTE_UP && !userId.equals(fromId))) {
				} else {
