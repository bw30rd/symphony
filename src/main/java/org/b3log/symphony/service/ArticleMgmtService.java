...
public class ArticleMgmtService {
	...
	private static boolean tagExists(final String tagTitle, final List<JSONObject> tags) throws JSONException {
		for (final JSONObject tag : tags) {
			if (tag.optString(Tag.TAG_TITLE).equals(tagTitle)) {
				return true;
			}
		}
		...
		Symphonys.EXECUTOR_SERVICE.submit(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					if ((article.optInt(Article.ARTICLE_VIEW_CNT) >= 100
							&& article.optInt(Article.ARTICLE_COMMENT_CNT) >= 15
							&& article.optInt(Article.ARTICLE_GOOD_CNT) >= 30)
							|| article.optInt(Article.ARTICLE_COLLECT_CNT) >= 30) {
	 ...
	 *            the specified request json object, for example, "articleTitle":
	 *            "", "articleTags": "", "articleContent": "", "articleEditorType":
	 *            "", "articleAuthorId": "", "articleCommentable": boolean, //
	 *            optional, default to true "syncWithSymphonyClient": boolean, //
	 *            optional "clientArticleId": "", // optional
	 *            "clientArticlePermalink": "", // optional "isBroadcast": boolean,
	 *            // Client broadcast, optional "articleType": int, // optional,
	 *            default to 0 "articleRewardContent": "", // optional, default to
	 *            "" "articleRewardPoint": int, // optional, default to 0
	 *            "articleIP": "", // optional, default to "" "articleUA": "", //
	 *            optional, default to "" "articleAnonymous": int, // optional,
	 *            default to 0 (public) "articleAnonymousView": int // optional,
	 *            default to 0 (use global) , see {@link Article} for more details
	...
	public synchronized String addArticle(final JSONObject requestJSONObject) throws ServiceException {
		...
		try {
			...
			if (!fromClient && Article.ARTICLE_ANONYMOUS_C_PUBLIC == articleAnonymous) {
				...
				/*
				 * if (balance - sum < 0) { throw new
				 * ServiceException(langPropsService.get("insufficientBalanceLabel")); }
				 */
			}
		...
		try {
			...
			try {
				wechatProcessor.sendWechatArticleByTag(articleTags, article, articleId);
			} catch (Exception e1) {
				LOGGER.log(Level.ERROR, "send wechat article error", e1);
			}
			...
			if (!fromClient && Article.ARTICLE_ANONYMOUS_C_PUBLIC == articleAnonymous) {
				 ...
				 */
				pointacquireMgmtService.acquire(authorId, Pointtransfer.ID_C_SYS,
						Pointtransfer.TRANSFER_TYPE_C_ADD_ARTICLE, Pointtransfer.TRANSFER_SUM_C_ADD_ARTICLE, articleId,
						System.currentTimeMillis());

				// Liveness
				livenessMgmtService.incLiveness(authorId, Pointtransfer.TRANSFER_SUM_C_ADD_ARTICLE);
	 ...
	 *
	 * @param requestJSONObject
	 *            the specified request json object, for example, "oId": "",
	 *            "articleTitle": "", "articleTags": "", "articleContent": "",
	 *            "articleEditorType": "", "articleCommentable": boolean, //
	 *            optional, default to true "clientArticlePermalink": "", //
	 *            optional "articleType": int // optional, default to 0
	 *            "articleRewardContent": "", // optional, default to ""
	 *            "articleRewardPoint": int, // optional, default to 0 "articleIP":
	 *            "", // optional, default to "" "articleUA": "", // optional
	 *            default to "" , see {@link Article} for more details
	 * @throws ServiceException
	 *             service exception
	 */
	public synchronized void updateArticle(final JSONObject requestJSONObject) throws ServiceException {
		...
		try {
			...
			if (0 < rewardPoint) {
				...
				oldArticle.put(Article.ARTICLE_REWARD_CONTENT,
						requestJSONObject.optString(Article.ARTICLE_REWARD_CONTENT));
			...
			if (!fromClient && Article.ARTICLE_ANONYMOUS_C_PUBLIC == articleAnonymous) {
				...
				/*
				 * if (enableReward) { pointtransferMgmtService.transfer(authorId,
				 * Pointtransfer.ID_C_SYS, Pointtransfer.TRANSFER_TYPE_C_ADD_ARTICLE_REWARD,
				 * Pointtransfer.TRANSFER_SUM_C_ADD_ARTICLE_REWARD, articleId,
				 * System.currentTimeMillis()); }
				 */
			}
...
//			}
			...
			if (Article.ARTICLE_PERFECT_C_NOT_PERFECT == oldArticle.optInt(Article.ARTICLE_PERFECT)
				 ...
				 * Pointtransfer.TRANSFER_TYPE_C_PERFECT_ARTICLE,
				 * Pointtransfer.TRANSFER_SUM_C_PERFECT_ARTICLE, articleId,
				 * System.currentTimeMillis());
				 */
				pointacquireMgmtService.acquire(authorId, Pointtransfer.ID_C_SYS,
				...
				// Liveness
				livenessMgmtService.incLiveness(authorId, Pointtransfer.TRANSFER_SUM_C_PERFECT_ARTICLE);
			}
	...
	public void updateAllArticlesByTag(String oldTag, String newTag) {
		List<JSONObject> articles = articleQueryService.getArticlesByTag(oldTag);

		final Transaction transaction = articleRepository.beginTransaction();
		String articleId = "";
		try {
			for (JSONObject article : articles) {
				articleId = article.optString(Keys.OBJECT_ID);
				article.put(Article.ARTICLE_TAGS, newTag);
	...
	public void reward(final String articleId, final String senderId) throws ServiceException {
		try {
			...
			/*
			 * if (Article.ARTICLE_ANONYMOUS_C_PUBLIC ==
			 * article.optInt(Article.ARTICLE_ANONYMOUS)) { final boolean succ = null !=
			 * pointtransferMgmtService.transfer(senderId, receiverId,
			 * Pointtransfer.TRANSFER_TYPE_C_ARTICLE_REWARD, rewardPoint, rewardId,
			 * System.currentTimeMillis());
			 * 
			 * if (!succ) { throw new ServiceException(); } }
			 */
	...
	public void thank(final String articleId, final String senderId) throws ServiceException {
		try {
			...
			if (Article.ARTICLE_ANONYMOUS_C_PUBLIC == article.optInt(Article.ARTICLE_ANONYMOUS)) {
				/*
				 * final boolean succ = null != pointtransferMgmtService.transfer(senderId,
				 * receiverId, Pointtransfer.TRANSFER_TYPE_C_ARTICLE_THANK,
				 * Pointtransfer.TRANSFER_SUM_C_ARTICLE_THANK, thankId,
				 * System.currentTimeMillis());
				 */

				final boolean succ = null != pointacquireMgmtService.acquire(receiverId, senderId,
			...
			// liveness
			livenessMgmtService.incLiveness(receiverId, Pointtransfer.TRANSFER_SUM_C_ARTICLE_THANK);
	...
	public synchronized void stick(final String articleId) throws ServiceException {
		...
		try {
			...
			/*
			 * final boolean succ = null !=
			 * pointtransferMgmtService.transfer(article.optString(Article.ARTICLE_AUTHOR_ID
			 * ), Pointtransfer.ID_C_SYS, Pointtransfer.TRANSFER_TYPE_C_STICK_ARTICLE,
			 * Pointtransfer.TRANSFER_SUM_C_STICK_ARTICLE, articleId,
			 * System.currentTimeMillis()); if (!succ) { throw new
			 * ServiceException(langPropsService.get("stickFailedLabel")); }
			 */
		} catch (final RepositoryException e) {
			if (transaction.isActive()) {
				transaction.rollback();
	 ...
	 *            "articleRewardContent": "", "articleRewardPoint": int, "userName":
	 *            "", "time": long , see {@link Article} for more details
	 * @return generated article id
	 * @throws ServiceException
	 *             service exception
	 */
