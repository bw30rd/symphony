...
public class UserQueryService {
	...
	public int getInvitedUserCount(final String userId) {
		final Query query = new Query().setFilter(
				CompositeFilterOperator.and(new PropertyFilter(Pointtransfer.TO_ID, FilterOperator.EQUAL, userId),
	...
	public JSONObject getLatestLoggedInUsers(final long time, final int currentPageNum, final int pageSize,
		...
		final Query query = new Query().addSort(Keys.OBJECT_ID, SortDirection.DESCENDING)
				...
				.setFilter(CompositeFilterOperator.and(
						new PropertyFilter(UserExt.USER_STATUS, FilterOperator.EQUAL, UserExt.USER_STATUS_C_VALID),
						new PropertyFilter(UserExt.USER_LATEST_LOGIN_TIME, FilterOperator.GREATER_THAN_OR_EQUAL,
								time)));
	...
	public void loadUserNames() {
		...
		query.addProjection(UserExt.USER_NICKNAME, String.class);
		...
		try {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				u.put(UserExt.USER_NICKNAME, user.optString(User.USER_NICKNAME));
	 ...
	 * @return a list of usernames, for example
	 * 
	 *         <pre>
	 * [
	 *     {
	...
	public List<JSONObject> getUserNamesByPrefix(final String namePrefix) {
		...
		while (start > -1 && USER_NAMES.get(start).optString(UserExt.USER_T_NAME_LOWER_CASE)
		...
		if (start < index - 5) {
			...
			while (end < USER_NAMES.size() && end < index + 5 && USER_NAMES.get(end)
					.optString(UserExt.USER_T_NAME_LOWER_CASE).startsWith(namePrefix.toLowerCase())) {
				end++;
	 ...
	 * 
	 * <pre>
	 * &#64;88250 It is a nice day. &#64;Vanessa, we are on the way.
	 * </pre>
	 * 
	...
	public Set<String> getUserNames(final String text) {
		...
		for (String maybeUserName : maybeUserNameSet) {
			...
			// }
			if (null != getUserByNameInCache(maybeUserName)) { // Found a user
				ret.add(maybeUserName);
			}
		}
	...
	/**
	 * Gets a user by a name in cache.
	 *
	 * @param name
	 *            the specified name
	 * @return user, returns {@code null} if not found
	 */
	public JSONObject getUserByNameInCache(final String name) {
		final JSONObject ret = userCache.getUserByName(name);
		return ret;
	}
	 ...
	 *            the specified request json object, for example, "userNameOrEmail":
	 *            "", // optional "paginationCurrentPageNum": 1,
	 *            "paginationPageSize": 20, "paginationWindowSize": 10, , see
...
//			user.put(Role.ROLE_NAME, role.optString(Role.ROLE_NAME));
			user.put(Role.ROLE_NAME, roleQueryService.getRoleNames(user.optString(User.USER_ROLE)));
			...
			user.put("followerCnt", followMgmtService.getFollowerCnt(user));
		}

		return ret;
	}

	public List<JSONObject> getUsersByNames(String names) {
		List<JSONObject> users = new ArrayList<JSONObject>();
		final Set<String> userNames = getUserNames(names);
		for (String userName : userNames) {
			userName = userName.trim();
			// JSONObject user = getUserByName(userName);
			JSONObject user = getUserByNameInCache(userName);
			users.add(user);
		}
		return users;
	 ...
	 *
	 * @param requestJSONObject
	 *            the specified request json object, for example, "userCity": "",
	 *            "userLatestLoginTime": long, // optional, default to 0
	 *            "paginationCurrentPageNum": 1, "paginationPageSize": 20,
	 *            "paginationWindowSize": 10, }, see {@link Pagination} for more
	 *            details
	 * @return for example,
	 * 
	 *         <pre>
	...
	public JSONObject getUsersByCity(final JSONObject requestJSONObject) throws ServiceException {
		...
		final Query query = new Query().addSort(UserExt.USER_LATEST_LOGIN_TIME, SortDirection.DESCENDING)
				.setCurrentPageNum(currentPageNum).setPageSize(pageSize).setFilter(
						CompositeFilterOperator.and(new PropertyFilter(UserExt.USER_CITY, FilterOperator.EQUAL, city),
		...
		try {
			for (int i = 0; i < users.length(); i++) {
				...
				users.getJSONObject(i).put(Common.IS_FOLLOWING,
						followRepository.exists(requestJSONObject.optString(Keys.OBJECT_ID),
	 ...
	 * @return
	 */
	public List<JSONObject> getUsersByLiveness(final int number, final String fromdate, final String todate) {
		try {
			final List<JSONObject> result = livenessRepository.select("SELECT\n"
					+ "	symphony_user.oId,userName,userNickname,SUM(livenessPoint) livenessPoint,userAvatarURL,userPoint,userIntro,userArticleCount,userTagCount\n"
					+ "FROM\n" + "symphony_liveness,symphony_user\n" + "WHERE\n" + "	livenessDate >= ?\n"
					+ "AND livenessDate <= ?\n" + "AND symphony_user.oId = symphony_liveness.livenessUserId\n"
					+ "GROUP BY\n" + "livenessUserId\n" + "ORDER BY\n" + "livenessPoint desc\n" + "", fromdate, todate);
			final List<JSONObject> userList = new ArrayList<>();
			if (!result.isEmpty()) {
				int count = 0;
				for (JSONObject user : result) {
					if (!("admin".equals(user.optString(User.USER_NAME)))) {
						user.put("userLevel", userLevelService.getUserLevel(user.getInt("userPoint")));
						user.put("userLevelType", userLevelService.getUserLevelType(user.getInt("userPoint")));
						userList.add(user);
						count++;
						if (count >= number) {
							return userList;
			...
			LOGGER.log(Level.ERROR, "get users by liveness error", e);
		}
		return null;
	}

	/***
	 * Gets users by rank.(No paging required)
	 * 
	 * @param number
	 * @return
	 */
	public List<JSONObject> getUsersByRank(final int number) throws ServiceException {
		try {
			final List<JSONObject> users = userRepository
			...
			LOGGER.log(Level.ERROR, "get users by rank error", e);
		}
		return null;
	}

	public JSONObject getUsersByRank(final JSONObject requestJSONObject) throws ServiceException {
		final JSONObject ret = new JSONObject();

		final int currentPageNum = requestJSONObject.optInt(Pagination.PAGINATION_CURRENT_PAGE_NUM);
		final int pageSize = requestJSONObject.optInt(Pagination.PAGINATION_PAGE_SIZE);
		final int windowSize = requestJSONObject.optInt(Pagination.PAGINATION_WINDOW_SIZE);
		final int number = requestJSONObject.optInt("number");

		final JSONArray users = CollectionUtils.listToJSONArray(getUsersByRank(number));
		final int pageCount = (users.length() - 1) / pageSize + 1;
	 ...
	 * @throws ServiceException
	 */
	public List<JSONObject> getUsers() throws ServiceException {
		try {
	...
	public List<JSONObject> getUsersByKeyword(String keyword) throws ServiceException {
		try {
			List<JSONObject> users = userRepository.select(
					"SELECT * FROM (SELECT *,LOCATE(?,userName) AS location FROM symphony_user WHERE userName LIKE CONCAT('%',?,'%')) AS users ORDER BY location asc",
					keyword, keyword);
			if (users.isEmpty()) {
				users = userRepository.select(
						"SELECT * FROM (SELECT *,LOCATE(?,userNickname) AS location FROM symphony_user WHERE userNickname LIKE CONCAT('%',?,'%')) AS users ORDER BY location asc",
						keyword, keyword);
			}
			final List<JSONObject> userList = new ArrayList<>();
			if (!users.isEmpty()) {
			...
			LOGGER.log(Level.ERROR, "get users by keyword error", e);
		}
		return null;
	}

	public JSONObject getUsersByKeyword(final JSONObject requestJSONObject) throws ServiceException {
		final JSONObject ret = new JSONObject();

		final int currentPageNum = requestJSONObject.optInt(Pagination.PAGINATION_CURRENT_PAGE_NUM);
		final int pageSize = requestJSONObject.optInt(Pagination.PAGINATION_PAGE_SIZE);
		final int windowSize = requestJSONObject.optInt(Pagination.PAGINATION_WINDOW_SIZE);
		final String keyword = requestJSONObject.optString("keyword");

		final JSONArray users = CollectionUtils.listToJSONArray(getUsersByKeyword(keyword));
		final int pageCount = (users.length() - 1) / pageSize + 1;
