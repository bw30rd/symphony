...
public class NotificationQueryService {
	...
	/**
	 * Gets the count of unread 'following' notifications of a user specified with
	 * the given user id.
	 *
	 * @param userId
	 *            the given user id
	...
	/**
	 * Gets the count of unread 'sys announce' notifications of a user specified
	...
	/**
	 * Gets 'sys announce' type notifications with the specified user id, current
	 * page number and page size.
	 *
	 * @param avatarViewMode
	 *            the specified avatar view mode
	 * @param userId
	 ...
	 *             service exception
	 */
	public JSONObject getSysAnnounceNotifications(final int avatarViewMode, final String userId,
		...
		try {
			...
			for (int i = 0; i < results.length(); i++) {
				...
				case Notification.DATA_TYPE_C_SYS_ANNOUNCE_ARTICLE:
					...
					final String articleLink15 = "<a href=\"" + article15.optString(Article.ARTICLE_PERMALINK) + "\">"
...
//					desTemplate = desTemplate.replace("{newRole}", newRole.optString(Role.ROLE_NAME));
					...
					desTemplate = desTemplate.replace("{oldRole}", roleQueryService.getRoleNames(oldRoleId));
					desTemplate = desTemplate.replace("{newRole}", roleQueryService.getRoleNames(newRoleId));
	...
	/**
	 * Gets the count of unread notifications of a user specified with the given
	 * user id.
	 *
	 * @param userId
	 *            the given user id
	...
	public int getUnreadNotificationCountByType(final String userId, final int notificationDataType) {
		...
		final Query query = new Query();
		query.setFilter(new CompositeFilter(CompositeFilterOperator.AND, filters)).addProjection(Keys.OBJECT_ID,
				String.class);
		...
		try {
			...
			LOGGER.log(Level.ERROR,
					"Gets [" + notificationDataType + "] notification count failed [userId=" + userId + "]", e);
	...
	/**
	 * Gets the count of unread 'point' notifications of a user specified with the
	 * given user id.
	 *
	 * @param userId
	 *            the given user id
	...
	public int getUnreadPointNotificationCount(final String userId) {
		...
		subFilters.add(new PropertyFilter(Notification.NOTIFICATION_DATA_TYPE, FilterOperator.EQUAL,
				Notification.DATA_TYPE_C_USER_TO_UPGRADE));
	...
	/**
	 * Gets 'point' type notifications with the specified user id, current page
			...
			for (int i = 0; i < results.length(); i++) {
				...
				case Notification.DATA_TYPE_C_POINT_ARTICLE_THANK:
					...
					final String userLink12 = "<a href=\"/member/" + user12.optString(User.USER_NAME) + "\">"
							+ user12.optString(User.USER_NICKNAME) + "</a>";
					desTemplate = desTemplate.replace("{user}", userLink12);

					final String articleLink12 = "<a href=\"" + article12.optString(Article.ARTICLE_PERMALINK) + "\">"
				...
				case Notification.DATA_TYPE_C_POINT_ARTICLE_REWARD:
					...
					final String userLink7 = "<a href=\"/member/" + user7.optString(User.USER_NAME) + "\">"
							+ user7.optString(User.USER_NICKNAME) + "</a>";
					desTemplate = desTemplate.replace("{user}", userLink7);

					final String articleLink7 = "<a href=\"" + article7.optString(Article.ARTICLE_PERMALINK) + "\">"
				...
				case Notification.DATA_TYPE_C_POINT_COMMENT_THANK:
					...
					final String userLink8 = "<a href=\"/member/" + user8.optString(User.USER_NAME) + "\">"
							+ user8.optString(User.USER_NICKNAME) + "</a>";
					desTemplate = desTemplate.replace("{user}", userLink8);

					final String articleLink8 = "<a href=\"" + article8.optString(Article.ARTICLE_PERMALINK) + "\">"
				...
				case Notification.DATA_TYPE_C_POINT_TRANSFER:
					...
					final String userLink101 = "<a href=\"/member/" + user101.optString(User.USER_NAME) + "\">"
							+ user101.optString(User.USER_NICKNAME) + "</a>";
				...
				case Notification.DATA_TYPE_C_INVITECODE_USED:
					...
					final String invitedUserLink = "<a href=\"/member/" + invitedUser.optString(User.USER_NAME) + "\">"
							+ invitedUser.optString(User.USER_NICKNAME) + "</a>";
				...
				case Notification.DATA_TYPE_C_INVITATION_LINK_USED:
					...
					final String invitedUserLink18 = "<a href=\"/member/" + invitedUser18.optString(User.USER_NAME)
							+ "\">" + invitedUser18.optString(User.USER_NICKNAME) + "</a>";
				...
				case Notification.DATA_TYPE_C_POINT_PERFECT_ARTICLE:
					...
					final String articleLink22 = "<a href=\"" + article22.optString(Article.ARTICLE_PERMALINK) + "\">"
				...
				case Notification.DATA_TYPE_C_USER_TO_UPGRADE:
					desTemplate = langPropsService.get("notificationUserUpgradeLabel");
					...
					String leveltype29 = "<span class=" + userLevelType29 + ">" + userLevel29 + "</span>";

					desTemplate = desTemplate.replace("{userLevel}", leveltype29);
					break;
				default:
					throw new AssertionError();
				}
	...
	/**
	 * Gets 'commented' type notifications with the specified user id, current page
	 * number and page size.
	 *
	 * @param avatarViewMode
	 *            the specified avatar view mode
	 ...
	 *             service exception
	 */
	public JSONObject getCommentedNotifications(final int avatarViewMode, final String userId, final int currentPageNum,
		...
		final List<Filter> filters = new ArrayList<>();
		filters.add(new PropertyFilter(Notification.NOTIFICATION_USER_ID, FilterOperator.EQUAL, userId));
		filters.add(new PropertyFilter(Notification.NOTIFICATION_DATA_TYPE, FilterOperator.EQUAL,
				Notification.DATA_TYPE_C_COMMENTED));
		...
		try {
			...
			for (int i = 0; i < results.length(); i++) {
				...
				if (comment != null) {
					...
					commentedNotification.put(Comment.COMMENT_T_AUTHOR_NAME,
					...
					commentedNotification.put(Common.THUMBNAIL_UPDATE_TIME,
							comment.optJSONObject(Comment.COMMENT_T_COMMENTER).optLong(UserExt.USER_UPDATE_TIME));
					...
					commentedNotification.put(Notification.NOTIFICATION_HAS_READ,
					...
					commentedNotification.put(Comment.COMMENT_SHARP_URL,
							"/article/" + articleId + "?p=" + cmtPage + "&m=" + cmtViewMode + "#" + commentId);
	...
	/**
	 * Gets 'reply' type notifications with the specified user id, current page
	 * number and page size.
	 *
	 * @param avatarViewMode
	 *            the specified avatar view mode
	 ...
	 *             service exception
	 */
	public JSONObject getReplyNotifications(final int avatarViewMode, final String userId, final int currentPageNum,
			final int pageSize) throws ServiceException {
		final JSONObject ret = new JSONObject();
		final List<JSONObject> rslts = new ArrayList<>();
		...
		filters.add(new PropertyFilter(Notification.NOTIFICATION_USER_ID, FilterOperator.EQUAL, userId));
		filters.add(new PropertyFilter(Notification.NOTIFICATION_DATA_TYPE, FilterOperator.EQUAL,
				Notification.DATA_TYPE_C_REPLY));
	...
	/**
	 * Gets 'at' type notifications with the specified user id, current page number
	 * and page size.
	 *
	 * @param avatarViewMode
	 *            the specified avatar view mode
	 ...
	 *             service exception
	 */
	public JSONObject getAtNotifications(final int avatarViewMode, final String userId, final int currentPageNum,
			final int pageSize) throws ServiceException {
		final JSONObject ret = new JSONObject();
		final List<JSONObject> rslts = new ArrayList<>();
		...
		try {
			...
			for (int i = 0; i < results.length(); i++) {
				...
				atNotification.put(Notification.NOTIFICATION_HAS_READ,
						notification.optBoolean(Notification.NOTIFICATION_HAS_READ));
				...
				case Notification.DATA_TYPE_C_AT:
					...
					if (null != comment) {
						...
						atNotification.put(Common.THUMBNAIL_URL,
								comment.optString(Comment.COMMENT_T_AUTHOR_THUMBNAIL_URL));
						atNotification.put(Common.THUMBNAIL_UPDATE_TIME,
								comment.optJSONObject(Comment.COMMENT_T_COMMENTER).optLong(UserExt.USER_UPDATE_TIME));
						...
						final String thumbnailURL = avatarQueryService.getAvatarURLByUser(avatarViewMode, articleAuthor,
								"48");
						atNotification.put(Common.THUMBNAIL_URL, thumbnailURL);
						atNotification.put(Common.THUMBNAIL_UPDATE_TIME,
								articleAuthor.optLong(UserExt.USER_UPDATE_TIME));
						atNotification.put(Article.ARTICLE_TITLE,
				...
				case Notification.DATA_TYPE_C_ARTICLE_NEW_WATCHER:
					...
					final String followerUserNickname = followerUser.optString(User.USER_NICKNAME);
					atNotification.put(User.USER_NAME, followerUserName);

					final String thumbnailURL = avatarQueryService.getAvatarURLByUser(avatarViewMode, followerUser,
							"48");
					atNotification.put(Common.THUMBNAIL_URL, thumbnailURL);
					atNotification.put(Common.THUMBNAIL_UPDATE_TIME, followerUser.optLong(UserExt.USER_UPDATE_TIME));

					final String userLink = "<a href=\"/member/" + followerUserName + "\">" + followerUserNickname
							+ "</a> ";
				...
				case Notification.DATA_TYPE_C_ARTICLE_VOTE_DOWN:
					...
					final String voterUserNickname = voter.optString(User.USER_NICKNAME);
					...
					final String userLinkVote = "<a href=\"/member/" + voterUserName + "\">" + voterUserNickname
							+ "</a> ";
	...
	/**
	 * Gets 'following' type notifications with the specified user id, current page
	 * number and page size.
	 *
	 * @param avatarViewMode
	 *            the specified avatar view mode
	 ...
	 *             service exception
	 */
	public JSONObject getFollowingNotifications(final int avatarViewMode, final String userId, final int currentPageNum,
			final int pageSize) throws ServiceException {
		final JSONObject ret = new JSONObject();
		final List<JSONObject> rslts = new ArrayList<>();
			...
			for (int i = 0; i < results.length(); i++) {
				...
				if (null != comment) {
					...
					followingNotification.put(Common.THUMBNAIL_URL,
							comment.optString(Comment.COMMENT_T_AUTHOR_THUMBNAIL_URL));
					followingNotification.put(Common.THUMBNAIL_UPDATE_TIME,
							comment.optJSONObject(Comment.COMMENT_T_COMMENTER).optLong(UserExt.USER_UPDATE_TIME));
					...
					followingNotification.put(Notification.NOTIFICATION_HAS_READ,
							notification.optBoolean(Notification.NOTIFICATION_HAS_READ));
					...
					followingNotification.put(Common.THUMBNAIL_UPDATE_TIME,
							articleAuthor.optLong(UserExt.USER_UPDATE_TIME));
					followingNotification.put(Article.ARTICLE_TITLE,
							Emotions.convert(article.optString(Article.ARTICLE_TITLE)));
					...
					followingNotification.put(Common.CREATE_TIME,
							new Date(article.optLong(Article.ARTICLE_CREATE_TIME)));
					followingNotification.put(Notification.NOTIFICATION_HAS_READ,
							notification.optBoolean(Notification.NOTIFICATION_HAS_READ));
	...
	/**
	 * Gets 'broadcast' type notifications with the specified user id, current page
	 * number and page size.
	 *
	 * @param avatarViewMode
	 *            the specified avatar view mode
	 ...
	 *             service exception
	 */
	public JSONObject getBroadcastNotifications(final int avatarViewMode, final String userId, final int currentPageNum,
			final int pageSize) throws ServiceException {
		final JSONObject ret = new JSONObject();
		final List<JSONObject> rslts = new ArrayList<>();
		...
		filters.add(new PropertyFilter(Notification.NOTIFICATION_USER_ID, FilterOperator.EQUAL, userId));
		filters.add(new PropertyFilter(Notification.NOTIFICATION_DATA_TYPE, FilterOperator.EQUAL,
				Notification.DATA_TYPE_C_BROADCAST));
