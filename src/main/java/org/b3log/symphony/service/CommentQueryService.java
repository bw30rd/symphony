...
public class CommentQueryService {
	 ...
	 * @param sortMode
	 *            the specified sort mode
	 * @param pageSize
	 *            the specified comment page size
	 ...
	 * @param commentViewMode
	 *            the specified comment view mode
	 * @param commentId
	...
	public JSONObject getOriginalComment(final int avatarViewMode, final int commentViewMode, final String commentId) {
		try {
			...
			ret.put(Pagination.PAGINATION_CURRENT_PAGE_NUM, getCommentPage(
					comment.optString(Comment.COMMENT_ON_ARTICLE_ID), commentId, commentViewMode, pageSize));
	...
	public List<JSONObject> getReplies(final int avatarViewMode, final int commentViewMode, final String commentId) {
		final Query query = new Query().addSort(Keys.OBJECT_ID, SortDirection.DESCENDING).setPageSize(Integer.MAX_VALUE)
				.setCurrentPageNum(1).setPageCount(1)
				.setFilter(CompositeFilterOperator.and(
						new PropertyFilter(Comment.COMMENT_ORIGINAL_COMMENT_ID, FilterOperator.EQUAL, commentId),
						new PropertyFilter(Comment.COMMENT_STATUS, FilterOperator.EQUAL,
								Comment.COMMENT_STATUS_C_VALID)));
		try {
			final List<JSONObject> comments = CollectionUtils
					.jsonArrayToList(commentRepository.get(query).optJSONArray(Keys.RESULTS));
			...
			for (final JSONObject comment : comments) {
				...
				reply.put(Comment.COMMENT_T_AUTHOR_THUMBNAIL_URL,
				...
				reply.put(Pagination.PAGINATION_CURRENT_PAGE_NUM,
						getCommentPage(comment.optString(Comment.COMMENT_ON_ARTICLE_ID),
								reply.optString(Keys.OBJECT_ID), commentViewMode, pageSize));
			}
	 ...
	 */
	public List<JSONObject> getNiceComments(final int avatarViewMode, final int commentViewMode, final String articleId,
			final int fetchSize) {
			...
			try {
				...
				for (final JSONObject comment : ret) {
					comment.put(Pagination.PAGINATION_CURRENT_PAGE_NUM,
							getCommentPage(articleId, comment.optString(Keys.OBJECT_ID), commentViewMode, pageSize));

					final JSONObject commenter = comment.optJSONObject("commenter");
					commenter.put("userLevel", userLevelService.getUserLevel(commenter.optInt("userPoint")));
					commenter.put("userLevelType", userLevelService.getUserLevelType(commenter.optInt("userPoint")));
	...
	/**
	 * Gets a comment with {@link #organizeComment(int, JSONObject)} by the
	 ...
	 * @param anonymous
	 *            the specified comment anonymous
	 * @param currentPageNum
	 *            the specified page number
	 * @param pageSize
	 *            the specified page size
	 * @param viewer
	 *            the specified viewer, may be {@code null}
			...
			for (final JSONObject comment : ret) {
				...
				if (Article.ARTICLE_ANONYMOUS_C_PUBLIC == article.optInt(Article.ARTICLE_ANONYMOUS)) {
					...
					comment.put(Comment.COMMENT_T_ARTICLE_AUTHOR_URL,
							"/member/" + articleAuthor.optString(User.USER_NAME));
					final String articleAuthorThumbnailURL = avatarQueryService.getAvatarURLByUser(avatarViewMode,
							articleAuthor, "48");
					...
					comment.put(Comment.COMMENT_T_ARTICLE_AUTHOR_URL, "");
					comment.put(Comment.COMMENT_T_ARTICLE_AUTHOR_THUMBNAIL_URL,
							avatarQueryService.getDefaultAvatarURL("48"));
				}
	...
	public List<JSONObject> getArticleComments(final String articleId) {
		try {
			final List<JSONObject> comments = commentRepository
					.select("SELECT * FROM symphony_comment WHERE commentOnArticleId = ?", articleId);

			final JSONObject article = articleRepository.get(articleId);

			for (JSONObject comment : comments) {
				organizeComments(0, comments);
				comment.put(Comment.COMMENT_T_ARTICLE_TITLE,
						Article.ARTICLE_STATUS_C_INVALID == article.optInt(Article.ARTICLE_STATUS)
								? langPropsService.get("articleTitleBlockLabel")
								: Emotions.convert(article.optString(Article.ARTICLE_TITLE)));
			...
			LOGGER.log(Level.ERROR, "get articles comments by tag error", e);
		}
		return null;
	}

	/**
	 * Gets the article comments with the specified article id, page number and page
	 * size.
	 *
	 * @param avatarViewMode
	 *            the specified avatar view mode
	 ...
	 *            the specified page size
	 * @param sortMode
	 *            the specified sort mode (traditional: 0, real time: 1)
	 * @return comments, return an empty list if not found
	 * @throws ServiceException
	 *             service exception
	 */
	public List<JSONObject> getArticleComments(final int avatarViewMode, final String articleId,
			final int currentPageNum, final int pageSize, final int sortMode) throws ServiceException {
		Stopwatchs.start("Get comments");

		final Query query = new Query().setPageCount(1).setCurrentPageNum(currentPageNum).setPageSize(pageSize)
	 ...
	 *            details
	 * @param commentFields
	 *            the specified article fields to return
	 * @return for example,
	 * 
	 *         <pre>
	 * {
	 ...
	 * @see Pagination
	 */
	public JSONObject getComments(final int avatarViewMode, final JSONObject requestJSONObject,
			final Map<String, Class<?>> commentFields) throws ServiceException {
		final JSONObject ret = new JSONObject();
	 ...
	 *
	 * @param comment
	 *            the specified comment, for example, "commentContent": "", ....,
	 *            "commenter": {}
	 */
	 ...
	 * @author xuchang
	 */
	public String getCommentMetaDesc(final JSONObject comment) {
		final String commentId = comment.optString(Keys.OBJECT_ID);
		...
		try {
			...
			try {
				...
				LOGGER.log(Level.ERROR, "Parses comment failed [id=" + commentId + ", md=" + ret + "]");
				throw e;
			}
