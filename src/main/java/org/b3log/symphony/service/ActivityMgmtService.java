...
public class ActivityMgmtService {
	...
	public synchronized JSONObject startEatingSnake(final String userId) {
		...
		/*
		 * final boolean succ = null != pointtransferMgmtService.transfer(userId,
		 * Pointtransfer.ID_C_SYS, Pointtransfer.TRANSFER_TYPE_C_ACTIVITY_EATINGSNAKE,
...
//			livenessMgmtService.incLiveness(userId, Liveness.LIVENESS_ACTIVITY);
	 ...
	 *            the specified user id
	 * @param characterImg
	 *            the specified character image encoded by Base64
	 * @param character
	 *            the specified character
	 * @return recognition result
	 */
	public synchronized JSONObject submitCharacter(final String userId, final String characterImg,
		...
		if (StringUtils.equals(character, recognizedCharacter)) {
			...
			query.setFilter(CompositeFilterOperator.and(
					new PropertyFilter(org.b3log.symphony.model.Character.CHARACTER_USER_ID, FilterOperator.EQUAL,
							userId),
					new PropertyFilter(org.b3log.symphony.model.Character.CHARACTER_CONTENT, FilterOperator.EQUAL,
							character)));
			...
			try {
				...
				LOGGER.log(Level.ERROR, "Count characters failed [userId=" + userId + ", character=" + character + "]",
						e);
			...
			/*
			 * pointtransferMgmtService.transfer(Pointtransfer.ID_C_SYS, userId,
			 * Pointtransfer.TRANSFER_TYPE_C_ACTIVITY_CHARACTER,
			 * Pointtransfer.TRANSFER_SUM_C_ACTIVITY_CHARACTER, characterId,
			 * System.currentTimeMillis());
			 */
	 ...
	 * @param userId
	 *            the specified user id
	 * @return {@code Random int} if checkin succeeded, returns
	 *         {@code Integer.MIN_VALUE} otherwise
	 */
	public synchronized int dailyCheckin(final String userId) {
		...
		// final int sum =
		// random.nextInt(Pointtransfer.TRANSFER_SUM_C_ACTIVITY_CHECKIN_MAX)
		 ...
		 * System.currentTimeMillis()); if (!succ) { return Integer.MIN_VALUE; }
		 */

		// pointacquireMgmtService.acquire(userId,Pointtransfer.ID_C_SYS,
		// Pointtransfer.TRANSFER_TYPE_C_ACTIVITY_CHECKIN, sum, userId,
		// System.currentTimeMillis());
		...
		try {
			...
			/** 8号：连续签到 */
			/*
			 * if (currentStreakDays > 0 && 0 == currentStreakDays % 10) { // Additional
			 * Point pointtransferMgmtService.transfer(Pointtransfer.ID_C_SYS, userId,
			...
			pointacquireMgmtService.acquire(userId, Pointtransfer.ID_C_SYS,
					Pointtransfer.TRANSFER_TYPE_C_ACTIVITY_CHECKIN, acquiresum, userId, System.currentTimeMillis());

			// Liveness
//			livenessMgmtService.incLiveness(userId, Liveness.LIVENESS_ACTIVITY);
			livenessMgmtService.incLiveness(userId, acquiresum);
	...
	public synchronized JSONObject bet1A0001(final String userId, final int amount, final int smallOrLarge) {
		...
		/*
		 * final boolean succ = null != pointtransferMgmtService.transfer(userId,
		 * Pointtransfer.ID_C_SYS, Pointtransfer.TRANSFER_TYPE_C_ACTIVITY_1A0001,
		 * amount, date + "-" + smallOrLarge, System.currentTimeMillis());
		 */
...
//			livenessMgmtService.incLiveness(userId, Liveness.LIVENESS_ACTIVITY);
	...
	public synchronized void yesterdayLivenessReward(final String userId) {
		 ...
		 * boolean succ = null !=
		 * pointtransferMgmtService.transfer(Pointtransfer.ID_C_SYS, userId,
		 * Pointtransfer.TRANSFER_TYPE_C_ACTIVITY_YESTERDAY_LIVENESS_REWARD, sum,
		 * userId, System.currentTimeMillis()); if (!succ) { return; }
		 */
...
//		livenessMgmtService.incLiveness(userId, Liveness.LIVENESS_ACTIVITY);
	...
	public synchronized JSONObject startGobang(final String userId) {
		...
		/*
		 * final boolean succ = null != pointtransferMgmtService.transfer(userId,
		 * Pointtransfer.ID_C_SYS, Pointtransfer.TRANSFER_TYPE_C_ACTIVITY_GOBANG,
		 * startPoint, "", System.currentTimeMillis());
		 */
		final boolean succ = true;
