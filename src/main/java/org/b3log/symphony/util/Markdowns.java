...
public final class Markdowns {
	 ...
	 */
	private static final int MD_TIMEOUT = 8000;

	/**
	 * Marked engine serve path.
	 */
	private static String MARKED_ENGINE_URL = "http://localhost:8250";
	...
	static {
		try {
			MARKED_ENGINE_URL = System.getenv("MARKED_ENGINE_URL");
			LOGGER.log(Level.INFO, "MARKED_ENGINE_URL=" + MARKED_ENGINE_URL);
			...
			LOGGER.log(Level.INFO,
					"[marked] is not available, uses built-in [pegdown] for markdown processing: " + e.getMessage());
		}
	}
	 ...
	 * @param baseURI
	 *            the specified base URI, the relative path value of href will
	 *            starts with this URL
	...
	public static String clean(final String content, final String baseURI) {
		...
		final String tmp = Jsoup.clean(content, baseURI, Whitelist.relaxed()
				...
				.addAttributes("audio", "controls", "src").addAttributes("video", "controls", "src", "width", "height")
				.addAttributes("source", "src", "media", "type")
				.addAttributes("object", "width", "height", "data", "type").addAttributes("param", "name", "value")
				.addAttributes("embed", "src", "type", "width", "height", "wmode", "allowNetworking"), outputSettings);
		...
		for (final Element iframe : iframes) {
			String src = StringUtils.deleteWhitespace(iframe.attr("src"));
			if (StringUtils.startsWithIgnoreCase(src, "javascript") || StringUtils.startsWithIgnoreCase(src, "data:")) {
				iframe.remove();
			} else if (StringUtils.startsWithIgnoreCase(src, "http://")) {
				if ("https".equals(Latkes.getServerScheme())) {
					src = StringUtils.replaceOnce(src, "http://", "https://");
					iframe.attr("src", src);
				}
			}
		}
	 ...
	 * @return converted HTML, returns an empty string "" if the specified markdown
	 *         text is "" or {@code null}, returns 'markdownErrorLabel' if exception
	 */
	public static String toHTML(final String markdownText) {
							...
							if (!parentElem.tagName().equals("code")) {
								...
								if (null != userQueryService) {
									try {
										...
										final List<JSONObject> users = userQueryService.getUsersByNames(text);

										for (final JSONObject user : users) {
											String userName = user.optString("userName");
											String userNickname = user.optString("userNickname");
											text = text.replace('@' + userName + " ",
													"@<a href='" + Latkes.getServePath() + "/member/" + userName + "'>"
															+ userNickname + "</a> ");
										}
		...
		try {
			...
			LOGGER.log(Level.ERROR, "Markdown timeout");
	...
	private static class ToHtmlSerializer implements Visitor {
		...
		public void visit(ListItemNode node) {
			if (node instanceof TaskListNode) {
				// vsch: #185 handle GitHub style task list items, these are a bit messy because
				// the <input> checkbox needs to be
				// included inside the optional <p></p> first grand-child of the list item,
				// first child is always RootNode
				...
				if (firstIsPara) {
					...
					printer.print("<input type=\"checkbox\" class=\"task-list-item-checkbox\""
							+ (((TaskListNode) node).isDone() ? " checked=\"checked\"" : "")
				...
				printer.indent(-indent).printchkln(indent != 0).print("</li>").printchkln(startWasNewLine);
			} else {
