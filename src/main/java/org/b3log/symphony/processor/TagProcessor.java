...
public class TagProcessor {
	...
	public void queryTags(final HTTPRequestContext context, final HttpServletRequest request,
	...
	public void cacheIconTags(final HttpServletRequest request, final HttpServletResponse response,
	...
	public void showTagsWall(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
	...
	@Before(adviceClass = { StopwatchStartAdvice.class, AnonymousViewCheck.class })
	@After(adviceClass = { PermissionGrant.class, StopwatchEndAdvice.class })
	public void showTagArticles(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response, final String tagURI) throws Exception {
		final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
		context.setRenderer(renderer);
		...
		if (null != user) {
			...
			JSONObject domain = domainQueryService.getByURI(tagURI);
			isDomainAdmin = (user.optString(User.USER_ROLE).contains(domain.optString(Domain.DOMAIN_TYPE)));
//			isDomainAdmin = roleQueryService.getRoleNames((user.optString(User.USER_ROLE)))
//					.contains(tag.optString(Tag.TAG_TITLE));
		}
		...
		tag.put(Common.IS_DOMAIN_ADMIN, isDomainAdmin);
		...
		// }
		dataModel.put(Common.SELECTED, tagURI);

		final String tagId = tag.optString(Keys.OBJECT_ID);

		final List<JSONObject> relatedTags = tagQueryService.getRelatedTags(tagId,
				Symphonys.getInt("tagRelatedTagsCnt"));
		...
		final List<JSONObject> articles = articleQueryService.getArticlesByTag(avatarViewMode, sortMode, tag, pageNum,
				pageSize);
		...
		tag.put(Tag.TAG_T_PARTICIPANTS, (Object) tagQueryService.getParticipants(avatarViewMode, tagId,
				Symphonys.getInt("tagParticipantsCnt")));
		...
		dataModel.put(Common.CURRENT,
				StringUtils.substringAfter(URLDecoder.decode(request.getRequestURI()), "/tag/" + tagURI));
	}
}
