...
public class IndexProcessor {
	...
	public void showMDGuide(final HTTPRequestContext context, final HttpServletRequest request,
...
//		final List<JSONObject> recentArticles = articleQueryService.getIndexRecentArticles(avatarViewMode);
		...
		final JSONObject recentArticles_result = articleQueryService.getRecentArticles(avatarViewMode, 0, 1, pageSize);
		final List<JSONObject> recentArticles = (List<JSONObject>) recentArticles_result.get(Article.ARTICLES);
...
//			}
			...
			final String userRoleName = roleQueryService.getRoleNames((currentUser.optString(User.USER_ROLE)));
			...
			dataModel.put("userRoleName", userRoleName);

			final String userId = currentUser.optString(Keys.OBJECT_ID);

			final JSONObject followingArticlesResult = followQueryService.getWatchingArticles(avatarViewMode, userId, 1,
					pageSize);

			final List<JSONObject> followingArticles = (List<JSONObject>) followingArticlesResult.opt(Keys.RESULTS);
			dataModel.put(Common.FOLLOWING_ARTICLES, followingArticles);

			final List<JSONObject> followingTagArticles = articleQueryService.getFollowingTagArticles(avatarViewMode,
					userId, 1, pageSize);
			dataModel.put(Common.FOLLOWING_TAG_ARTICLES, followingTagArticles);

			final List<JSONObject> followingUserArticles = articleQueryService.getFollowingUserArticles(avatarViewMode,
					userId, 1, pageSize);
			dataModel.put(Common.FOLLOWING_USER_ARTICLES, followingUserArticles);
		} else {
			dataModel.put(Common.FOLLOWING_ARTICLES, Collections.emptyList());
		...
		// final List<JSONObject> perfectArticles =
		// articleQueryService.getIndexPerfectArticles(avatarViewMode);
		// dataModel.put(Common.PERFECT_ARTICLES, perfectArticles);

		// Get all the users to UserCache.
		if (userCache.isCacheEmpty()) {
			userQueryService.getUsers();
		}

		final JSONObject result = articleQueryService.getPerfectArticles(avatarViewMode, 1, pageSize);
	...
	/**
	 * get recent articles by the given page number.
	 *
	 * @param context
	 *            the specified context
	 * @param request
	 ...
	 *             exception
	 */
	@RequestProcessing(value = { "/recent/more" }, method = HTTPRequestMethod.GET)
	@Before(adviceClass = { StopwatchStartAdvice.class, AnonymousViewCheck.class })
	@After(adviceClass = { PermissionGrant.class, StopwatchEndAdvice.class })
	public void getMoreRecent(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		...
		final int avatarViewMode = (int) request.getAttribute(UserExt.USER_AVATAR_VIEW_MODE);
		int sortMode = 0;
		int pageSize = Symphonys.getInt("mobileIndexArticlesCnt");

		List<JSONObject> allArticles = new ArrayList<JSONObject>();
		if (StringUtils.equals(pageCount, "only")) {

			// Returns only the articles of the current page.
			final JSONObject result = articleQueryService.getRecentArticles(avatarViewMode, sortMode, pageNum,
					pageSize);
			allArticles = (List<JSONObject>) result.get(Article.ARTICLES);
		} else {

			// Returns the articles of the current and previous page.
			for (int i = 1; i <= pageNum; i++) {
				JSONObject result = articleQueryService.getRecentArticles(avatarViewMode, sortMode, i, pageSize);
				List<JSONObject> articles = (List<JSONObject>) result.get(Article.ARTICLES);
		...
		context.renderJSONValue(Common.LATEST_ARTICLES, allArticles);
	}
	 ...
	 *             exception
	 */
	@RequestProcessing(value = { "/recent", "/recent/hot", "/recent/good",
			"/recent/reply" }, method = HTTPRequestMethod.GET)
	@Before(adviceClass = { StopwatchStartAdvice.class, AnonymousViewCheck.class })
	@After(adviceClass = { PermissionGrant.class, StopwatchEndAdvice.class })
	public void showRecent(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
		context.setRenderer(renderer);
...
//			}
	...
	@Before(adviceClass = { StopwatchStartAdvice.class, AnonymousViewCheck.class })
	@After(adviceClass = { PermissionGrant.class, StopwatchEndAdvice.class })
	public void showHotArticles(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
		context.setRenderer(renderer);
	...
	@Before(adviceClass = { StopwatchStartAdvice.class, AnonymousViewCheck.class })
	@After(adviceClass = { PermissionGrant.class, StopwatchEndAdvice.class })
	public void showSymHub(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
		context.setRenderer(renderer);
	...
	@Before(adviceClass = { StopwatchStartAdvice.class, AnonymousViewCheck.class })
	@After(adviceClass = { PermissionGrant.class, StopwatchEndAdvice.class })
	public void showPerfectArticles(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
		context.setRenderer(renderer);
...
//			}
	...
	@Before(adviceClass = StopwatchStartAdvice.class)
	@After(adviceClass = { PermissionGrant.class, StopwatchEndAdvice.class })
	public void showB3log(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
		context.setRenderer(renderer);
