...
public class ActivityProcessor {
	...
	public void showCharacter(final HTTPRequestContext context, final HttpServletRequest request,
		...
		final String totalProgress = String.format("%.2f",
				(double) writtenCharacterCount / (double) totalCharacterCount * 100);
		...
		final String userProgress = String.format("%.2f",
				(double) userWrittenCharacterCount / (double) totalCharacterCount * 100);
	...
	public void showActivities(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
		context.setRenderer(renderer);
	...
	@Before(adviceClass = { StopwatchStartAdvice.class, LoginCheck.class })
	@After(adviceClass = { PermissionGrant.class, StopwatchEndAdvice.class })
	public void showDailyCheckin(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final JSONObject user = (JSONObject) request.getAttribute(User.USER);
		final String userId = user.optString(Keys.OBJECT_ID);
		if (activityQueryService.isCheckedinToday(userId)) {
			final String checkinType = request.getParameter("checkinType");
			if ("phone".equals(checkinType)) {
				response.sendRedirect(Latkes.getServePath() + "/member/" + user.optString(User.USER_NAME));
			} else {
		...
		// dataModelService.fillHeaderAndFooter(request, response, dataModel);
		//
		// final int avatarViewMode = (int)
		// request.getAttribute(UserExt.USER_AVATAR_VIEW_MODE);
		//
		// dataModelService.fillRandomArticles(avatarViewMode, dataModel);
		// dataModelService.fillSideHotArticles(avatarViewMode, dataModel);
		// dataModelService.fillSideTags(dataModel);
	 ...
	 *             exception
	 */
	@RequestProcessing(value = "/activity/daily-checkin", method = HTTPRequestMethod.POST)
	@Before(adviceClass = { StopwatchStartAdvice.class, LoginCheck.class, CSRFCheck.class })
	@After(adviceClass = StopwatchEndAdvice.class)
	public void dailyCheckin(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final JSONObject user = (JSONObject) request.getAttribute(User.USER);
		final String userId = user.optString(Keys.OBJECT_ID);
		...
		if (!Symphonys.getBoolean("geetest.enabled")) {
			...
			if (StringUtils.isBlank(challenge) || StringUtils.isBlank(validate) || StringUtils.isBlank(seccode)) {
				// response.sendRedirect(Latkes.getServePath() + "/member/" +
			...
			final int gt_server_status_code = (Integer) request.getSession()
		...
		final int userRank = userLevelService.getRanking(currentUser.optString(User.USER_NAME), avatarViewMode);

		context.renderJSON();
		context.renderJSONValue(User.USER_NAME, user.optString(User.USER_NAME));
		context.renderJSONValue(Keys.STATUS_CODE, StatusCodes.SUCC);
		context.renderJSONValue("currentUserLevel", currentUserLevel);
		context.renderJSONValue("currentUserLevelType", currentUserLevelType);
		context.renderJSONValue("upgradePoints", upgradePoints);
		context.renderJSONValue("userRank", userRank);
		context.renderJSONValue("userPoint", currentUser.optInt("userPoint"));
		context.renderJSONValue("checkedinStreak", currentUser.optInt(UserExt.USER_CURRENT_CHECKIN_STREAK));
		context.renderJSONValue("usedPoint", user.optInt("userPoint"));
	...
	@Before(adviceClass = { StopwatchStartAdvice.class, LoginCheck.class })
	@After(adviceClass = StopwatchEndAdvice.class)
	public void yesterdayLivenessReward(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final JSONObject user = (JSONObject) request.getAttribute(User.USER);
		final String userId = user.optString(Keys.OBJECT_ID);
	...
	@Before(adviceClass = { StopwatchStartAdvice.class, LoginCheck.class })
	@After(adviceClass = { CSRFToken.class, PermissionGrant.class, StopwatchEndAdvice.class })
	public void show1A0001(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
		context.setRenderer(renderer);
	...
	@Before(adviceClass = { StopwatchStartAdvice.class, LoginCheck.class, CSRFCheck.class,
			Activity1A0001Validation.class })
	@After(adviceClass = StopwatchEndAdvice.class)
	...
	@Before(adviceClass = { StopwatchStartAdvice.class, LoginCheck.class })
	@After(adviceClass = { CSRFToken.class, PermissionGrant.class, StopwatchEndAdvice.class })
	public void showEatingSnake(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
		context.setRenderer(renderer);
	...
	@Before(adviceClass = { StopwatchStartAdvice.class, LoginCheck.class })
	@After(adviceClass = { CSRFToken.class, PermissionGrant.class, StopwatchEndAdvice.class })
	public void showGobang(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
		context.setRenderer(renderer);
		...
		pointActivityGobang = pointActivityGobang.replace("{point}",
				String.valueOf(Pointtransfer.TRANSFER_SUM_C_ACTIVITY_GOBANG_START));
	...
	public void startGobang(final HTTPRequestContext context, final HttpServletRequest request,
		...
		final boolean succ = currentUser.optInt(UserExt.USER_POINT)
				- Pointtransfer.TRANSFER_SUM_C_ACTIVITY_GOBANG_START >= 0;
