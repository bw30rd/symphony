...
public class UserProcessor {
		...
		if (null == result) {
			...
			case Invitecode.STATUS_C_UNUSED:
				...
				msg = msg.replace("${time}", DateFormatUtils.format(
						result.optLong(Keys.OBJECT_ID) + Symphonys.getLong("invitecode.expired"), "yyyy-MM-dd HH:mm"));
	...
	public void pointBuy(final HTTPRequestContext context, final HttpServletRequest request,
		...
		final String transferId = pointtransferMgmtService.transfer(fromId, Pointtransfer.ID_C_SYS,
				Pointtransfer.TRANSFER_TYPE_C_BUY_INVITECODE, Pointtransfer.TRANSFER_SUM_C_BUY_INVITECODE, invitecode,
		...
		if (!succ) {
			...
			msg = msg.replace("${time}", DateFormatUtils
					.format(System.currentTimeMillis() + Symphonys.getLong("invitecode.expired"), "yyyy-MM-dd HH:mm"));
	...
	public void showSettings(final HTTPRequestContext context, final HttpServletRequest request,
		...
		final String currentUserLevel = userLevelService.getUserLevel(user.getInt("userPoint"));
		user.put("userLevel", currentUserLevel);
		final String currentUserLevelType = userLevelService.getUserLevelType(user.getInt("userPoint"));
		user.put("userLevelType", currentUserLevelType);

		// 获取当前用户收藏的帖子数,粉丝和关注的
		int followingArticleCnt = followMgmtService.getFollowCnt(user, Follow.FOLLOWING_TYPE_C_ARTICLE);
		int followerCnt = followMgmtService.getFollowerCnt(user);
		int followingUserCnt = followMgmtService.getFollowingUserCnt(user);
		dataModel.put("followingArticleCnt", followingArticleCnt);
		dataModel.put("followerCnt", followerCnt);
		dataModel.put("followingUserCnt", followingUserCnt);
	...
	public void showHomeAnonymousComments(final HTTPRequestContext context, final HttpServletRequest request,
		...
		final List<JSONObject> userComments = commentQueryService.getUserComments(avatarViewMode,
				user.optString(Keys.OBJECT_ID), Comment.COMMENT_ANONYMOUS_C_ANONYMOUS, pageNum, pageSize, currentUser);
	...
	@After(adviceClass = { CSRFToken.class, PermissionGrant.class, StopwatchEndAdvice.class })
	public void showHome(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response, final String userName) throws Exception {
		final JSONObject user = (JSONObject) request.getAttribute(User.USER);
	...
	@Before(adviceClass = { StopwatchStartAdvice.class, AnonymousViewCheck.class, UserBlockCheck.class })
	@After(adviceClass = { CSRFToken.class, PermissionGrant.class, StopwatchEndAdvice.class })
	public void showHomeComments(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response, final String userName) throws Exception {
		final JSONObject user = (JSONObject) request.getAttribute(User.USER);
	...
	public void showHomeFollowingUsers(final HTTPRequestContext context, final HttpServletRequest request,
		...
		final JSONObject followingUsersResult = followQueryService.getFollowingUsers(avatarViewMode, followingId,
				pageNum, pageSize);
		...
		final int followingUsersCnt = followingUsersResult.optInt(Pagination.PAGINATION_RECORD_COUNT);
		final int pageCount = (int) Math.ceil((double) followingUsersCnt / (double) pageSize);
		...
		dataModel.put(Pagination.PAGINATION_PAGE_COUNT, pageCount);
		dataModel.put(Pagination.PAGINATION_PAGE_NUMS, pageNums);
		dataModel.put(Pagination.PAGINATION_RECORD_COUNT, followingUsersCnt);
	...
	public void showHomeFollowingArticles(final HTTPRequestContext context, final HttpServletRequest request,
		...
		final JSONObject followingArticlesResult = followQueryService.getFollowingArticles(avatarViewMode, followingId,
				pageNum, pageSize);
		...
		if (isLoggedIn) {
			...
			for (final JSONObject followingArticle : followingArticles) {
				...
				followingArticle.put(Common.IS_FOLLOWING, followQueryService.isFollowing(followerId,
						homeUserFollowingArticleId, Follow.FOLLOWING_TYPE_C_ARTICLE));
			}
		}

		user.put(UserExt.USER_T_CREATE_TIME, new Date(user.getLong(Keys.OBJECT_ID)));

		final int followingArticlesCnt = followingArticlesResult.optInt(Pagination.PAGINATION_RECORD_COUNT);
		final int pageCount = (int) Math.ceil(followingArticlesCnt / (double) pageSize);
		...
		dataModel.put(Pagination.PAGINATION_PAGE_COUNT, pageCount);
		dataModel.put(Pagination.PAGINATION_PAGE_NUMS, pageNums);
		dataModel.put(Pagination.PAGINATION_RECORD_COUNT, followingArticlesCnt);
	...
	public void showHomeWatchingArticles(final HTTPRequestContext context, final HttpServletRequest request,
		...
		final JSONObject followingArticlesResult = followQueryService.getWatchingArticles(avatarViewMode, followingId,
				pageNum, pageSize);
		final List<JSONObject> followingArticles = (List<JSONObject>) followingArticlesResult.opt(Keys.RESULTS);
		dataModel.put(Common.USER_HOME_FOLLOWING_ARTICLES, followingArticles);
		...
		if (isLoggedIn) {
			...
			for (final JSONObject followingArticle : followingArticles) {
				...
				followingArticle.put(Common.IS_FOLLOWING, followQueryService.isFollowing(followerId,
						homeUserFollowingArticleId, Follow.FOLLOWING_TYPE_C_ARTICLE_WATCH));
			}
		}
	...
	public void showHomeFollowers(final HTTPRequestContext context, final HttpServletRequest request,
		...
		final JSONObject followerUsersResult = followQueryService.getFollowerUsers(avatarViewMode, followingId, pageNum,
				pageSize);
	...
	public void showHomePoints(final HTTPRequestContext context, final HttpServletRequest request,
		...
		final JSONObject userPointsResult = pointtransferQueryService.getUserPoints(user.optString(Keys.OBJECT_ID),
				pageNum, pageSize);
		final List<JSONObject> userPoints = CollectionUtils
				.<JSONObject>jsonArrayToList(userPointsResult.optJSONArray(Keys.RESULTS));
	...
	public void pointTransfer(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final JSONObject ret = Results.falseResult();
		context.renderJSON(ret);
	...
	public void syncUser(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		context.renderJSON();
		...
		if (UserRegisterValidation.invalidUserName(name)) {
			LOGGER.log(Level.WARN, "Sync add user[name={0}, host={1}] error, caused by the username is invalid", name,
					clientHost);
		...
		if (Networks.isIPv4(maybeIP)) {
			LOGGER.log(Level.WARN, "Sync add user[name={0}, host={1}] error, caused by the client host is IPv4", name,
					clientHost);
		...
		if (null == user) {
			...
			try {
				...
				LOGGER.log(Level.ERROR, "Sync add user[name={0}, email={1}, host={2}] error: {3}", name, email,
						clientHost, e.getMessage());
			}
	...
	public void resetUnverifiedUsers(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
	...
	public void listNames(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		...
		final String namePrefix = java.net.URLDecoder.decode(request.getParameter("name"), "UTF-8");
		...
		if (StringUtils.isBlank(namePrefix)) {
			...
			final String followingId = user.optString(Keys.OBJECT_ID);
			final int avatarViewMode = (int) request.getAttribute(UserExt.USER_AVATAR_VIEW_MODE);
			final JSONObject followingUsersResult = followQueryService.getFollowingUsers(avatarViewMode, followingId, 1,
					30);
			final List<JSONObject> admins = (List<JSONObject>) followingUsersResult.opt(Keys.RESULTS);
			...
			for (final JSONObject admin : admins) {
				...
				userName.put(User.USER_NICKNAME, admin.optString(User.USER_NICKNAME));
		...
		// final List<JSONObject> userNames =
		// userQueryService.getUsersByKeyword(namePrefix);
	...
	public void loadUserNames(final HTTPRequestContext context, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		final String key = Symphonys.get("keyOfSymphony");
		if (!key.equals(request.getParameter("key"))) {
			response.sendError(HttpServletResponse.SC_FORBIDDEN);
...
//		user.put(Role.ROLE_NAME, role.optString(Role.ROLE_NAME));
		user.put(Role.ROLE_NAME, roleQueryService.getRoleNames(roleId));
	}
}
